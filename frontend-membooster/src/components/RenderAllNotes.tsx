...
const RenderAllNotes = ({notes, filters}:RenderAllNotesProps) => {
  ...
  if(filters.searchingTitle?.length > 0 || filters.searchingTags?.length > 0){
     ...
     finalNotes = notes.filter((note)=> {
      ...
      if (filters.searchingTitle) {
        ...
        return (note.title.toLocaleLowerCase().includes(filters.searchingTitle.toLocaleLowerCase()) || findIntersection(note.tags, filters.searchingTags))}
  ...
  return (
    <>  <Box m={4} display={'flex'} gap={5}  flexDirection={'row'} flexWrap={'wrap'} justifyContent={'space-evenly'}>
            {
